...
function render(status) {
  ...
  const overrides = []
  if (settings.overrides.channel) overrides.push(`更新通道被环境变量覆盖为 ${settings.effective.channel}`)
  ...
  statusBox.className = `status ${state.lastError ? 'err' : state.pendingVersion ? 'warn' : 'ok'}`
